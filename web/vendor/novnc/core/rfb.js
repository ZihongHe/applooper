...
export default class RFB extends EventTargetMixin {
     ...
     */
    _fail(details) {
        this.dispatchEvent(new CustomEvent("rfbfailure",
                                           { detail: { details: String(details || "") } }));
        switch (this._rfbConnectionState) {
            case 'disconnecting':
    ...
    _framebufferUpdate() {
        ...
        this._display.flip();
        if (this._FBU.hadData && !this._firstFrameReceived && !this._firstFramePending) {
            this._firstFramePending = true;
            this._display.flush()
                .then(() => {
                    this._firstFramePending = false;
                    if (this._firstFrameReceived || this._rfbConnectionState !== "connected") {
                        return;
                    }
    ...
    _handleRect() {
        switch (this._FBU.encoding) {
            ...
            default:
                {
                    const handled = this._handleDataRect();
                    if (handled) {
                        this._FBU.hadData = true;
                    }
                    return handled;
                }
        }
